...
use crate::orderbook::{
    BboResponse, EventInfo, Orderbook, OrderbookResponse, ResponseMetadata, TokenSummary,
};
use dashmap::DashMap;
...
impl OrderbookStore {
    ...
    pub fn cache_aggregate_mapping(&self, platform: &str, slug: &str, event_aggregate_id: &str) {
        ...
        self.inner
            .aggregate_mapping_cache
            .insert(cache_key, event_aggregate_id.to_string());
    }
    ...
    pub fn cache_event_metadata(&self, event_aggregate_id: &str, metadata: EventMetadata) {
        self.inner
            ...
            .insert(event_aggregate_id.to_string(), metadata);
    }
    ...
    pub fn cache_market_metadata(&self, hashed_market_id: &str, metadata: MarketMetadata) {
        self.inner
            ...
            .insert(hashed_market_id.to_string(), metadata);
    }
    ...
    pub fn apply_with_aggregate(&self, msg: &NormalizedOrderbook, event_aggregate_id: &str) {
        ...
        let market_books = event_books.entry(hashed_market_id.clone()).or_default();
    ...
    pub fn get_aggregate(
        ...
        event_aggregate_id: &str,
        depth: Option<usize>,
    ) -> Option<EventOrderbooksResponse> {
        ...
        self.inner
            ...
            .map(|aggregate_books| {
                ...
                for market_entry in aggregate_books.iter() {
                    ...
                    let metadata =
                        self.build_response_metadata(event_aggregate_id, &hashed_market_id);

                    let tokens: Vec<OrderbookResponse> = market_entry
                        .value()
                        .iter()
    ...
    pub fn get_market_bbos(
        &self,
        event_aggregate_id: &str,
        hashed_market_id: &str,
    ) -> Option<MarketBbosResponse> {
        let metadata = self.build_response_metadata(event_aggregate_id, hashed_market_id);
    ...
    pub fn list_all(&self) -> AllOrderbooksResponse {
        let events: Vec<EventSummary> = self
            ...
            .map(|event_entry| {
                ...
                let markets: Vec<MarketSummary> = event_entry
                    .value()
                    .iter()
                    .map(|market_entry| {
                        let hashed_market_id = market_entry.key().clone();
                        ...
                        let token_summaries: Vec<TokenSummary> = market_entry
                            .value()
                            .iter()
